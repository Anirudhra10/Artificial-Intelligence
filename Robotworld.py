#Anirudh Ramesh Aekawadi
#anramesh@indiana.edu
    ...
    for action in curr[1].allowedActions():
        ...
        if(action != "backward"):
            if duplicateWorld(neighbor,visited) == False:
                heappush(queue,(len(queue),neighbor))
...
def depthFirst(queue,visited):
    ...
    for action in curr[1].allowedActions():
        ...
        if(action != "backward"): #in depth first search, level is added at every function call
            if duplicateWorld(neighbor,visited) == False:
                heappush(queue,(level + 1,neighbor))

    return queue,curr[1]    

def determine(curr,name,visited,heuristic,queue,search_technique):
    global captured
    arr =[]
    min = 1000
    ...
    for i in range (len(arr)):
        ...
        if curr[1].hand['held'] == None or arr[i].hand['held'] != None or curr[1].goal[name] == arr[i].hand['location']:

            if duplicateWorld(arr[i],visited) == False:
                if arr[i].hand['held'] != None:
                    temp = heuristic(arr[i].hand['location'],arr[i].goal[name])

                else:
                    temp = heuristic(arr[i].hand['location'],arr[i].blocks[name])

                if search_technique == 'a_star': #deltacost is added only to A-Star search
                    temp = temp + deltaCost
...
def initiate(queue,visited,heuristic,search_technique):
    global captured, done, counter,bfs
    name = []
    bfs = True
    ...
    if len(name[0]) > counter:
        determine(curr, name[0][counter],visited,heuristic,queue,search_technique)

    return curr[1]

def bestFirst(queue,visited,heuristic):
    ret_val = initiate(queue,visited,heuristic,'best_first')   
    return queue,ret_val
    

def aStar(queue,visited,heuristic):
    ret_val = initiate(queue,visited,heuristic,'a_star')   
    return queue,ret_val
...
hs = {manhattan, euclidean} #this is a set, put your heuristic functions in here.
presolved = RobotWorld(4,4,1,{'A' : (3,3,0), 'B' : (3,1,0)}, {'A' : (3,3,0), 'B' : (3,1,0)},0)

easy = RobotWorld(5,5,1,{'A' : (1,0,0), 'B' : (3,1,0)}, {'A' : (1,1,0), 'B' : (3,1,0)},0)

medium = RobotWorld(6,6,1,{'A' : (1,1,0), 'B' : (3,1,0)}, {'A' : (4,4,0), 'B' : (4,5,0)},1)

hard = RobotWorld(10,10,1,{'A' : (1,0,0), 'B' : (9,9,0), 'C' : (4,4,0)}, {'A' : (4,4,0), 'B' : (1,0,0), 'C' : (9,9,0)},1)
